...
import { Search, Navigation, AlertTriangle, CheckCircle2, ShieldAlert, Droplets, MapPin, Users, Activity, Sparkles } from 'lucide-react';
    ...
    <section className="pt-6 pb-4 px-4 sm:px-6 lg:px-8 max-w-[1536px] mx-auto transition-colors">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm sm:text-base text-slate-600 dark:text-slate-300 font-normal leading-relaxed max-w-3xl">
        ...
        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm flex-shrink-0 self-start lg:self-auto">
          <div className="w-9 h-9 rounded-md bg-sky-50 dark:bg-sky-950/80 border border-sky-200 dark:border-sky-800 flex items-center justify-center text-sky-700 dark:text-sky-300">
            <Activity className="w-5 h-5 animate-pulse" />
          </div>
          <div className="text-xs">
            <div className="font-bold text-[#0b1c30] dark:text-slate-100 flex items-center gap-1.5">
              <span>Active Hydrologic Watch</span>
              <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
            </div>
            <div className="text-slate-500 dark:text-slate-400 font-mono text-[11px]">
              Assam Valley • {Object.keys(ASSAM_SECTORS).length} Monitored Catchments
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-3.5 shadow-sm hover:border-slate-300 dark:hover:border-slate-700 transition">
          <div className="flex items-center justify-between gap-1 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
            ...
            <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase ${
              currentSector.hazardLevel === 'CRITICAL' || currentSector.hazardLevel === 'HIGH' ? 'bg-red-100 dark:bg-red-950/80 text-red-700 dark:text-red-300' :
