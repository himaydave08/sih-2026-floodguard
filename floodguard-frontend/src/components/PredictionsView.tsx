...
export const PredictionsView: React.FC<PredictionsViewProps> = ({
  ...
  return (
    <div className="max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-5 animate-in fade-in duration-200">
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-300 dark:border-slate-800 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4 transition-colors">
        <div>
        ...
        {/* Sector Selector */}
        <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/80 p-2 rounded-lg border border-slate-200 dark:border-slate-700 overflow-x-auto self-start lg:self-auto">
          <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase font-mono mr-1">Catchment:</span>
          {Object.values(ASSAM_SECTORS).map((s) => (
            <button
              key={s.id}
              onClick={() => onSelectSector(s.id)}
              className={`px-3 py-1.5 text-xs rounded-md font-semibold transition whitespace-nowrap ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-300 dark:border-slate-800 shadow-sm space-y-3 transition-colors">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-heading font-bold text-sm text-[#0b1c30] dark:text-slate-100 uppercase tracking-wider font-mono flex items-center gap-1.5">
                <Waves className="w-4 h-4 text-sky-600 dark:text-sky-400" />
                <span>River Stage Hydrograph vs Danger Mark</span>
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-mono mt-0.5">
                {currentSector.riverName} at {currentSector.stationName} ({currentSector.stationCode})
              </p>
            </div>
            <span className="text-xs font-bold font-mono text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/70 px-2 py-1 rounded border border-red-200 dark:border-red-900">
              Peak: +12 Hours
            </span>
          </div>
          ...
          <div className="p-2.5 bg-slate-50 dark:bg-slate-800/80 rounded-lg text-xs text-slate-600 dark:text-slate-300 flex justify-between font-mono">
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-300 dark:border-slate-800 shadow-sm space-y-3 transition-colors">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-heading font-bold text-sm text-[#0b1c30] dark:text-slate-100 uppercase tracking-wider font-mono flex items-center gap-1.5">
                <Droplets className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                <span>Heavy Rainfall Hyetograph & Accumulation</span>
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-mono mt-0.5">
                IMD Telemetric Precipitation (Past 24h & 72h Ensemble)
              </p>
            </div>
            <span className="text-xs font-bold font-mono text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/70 px-2 py-1 rounded border border-blue-200 dark:border-blue-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-300 dark:border-slate-800 shadow-sm space-y-3 transition-colors">
          <div className="flex items-center justify-between">
            <h3 className="font-heading font-bold text-sm text-[#0b1c30] dark:text-slate-100 uppercase tracking-wider font-mono">
              72-Hour Flood Inundation Area Progression (km²)
            </h3>
            <span className="text-xs text-slate-400 dark:text-slate-500 font-mono">Spatial-Temporal ConvLSTM</span>
          </div>
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-300 dark:border-slate-800 shadow-sm space-y-3 flex flex-col justify-between transition-colors">
          <div>
